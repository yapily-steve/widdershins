...
function convertToToc(source, data) {
    ...
    for (var p in source.paths) {
        if (source.paths[p].hasOwnProperty('x-private') && source.paths[p]["x-private"]) {
            console.log("skipping x-private endpoint: ", p);
            continue;
        }
        if (!p.startsWith('x-')) {
            ...
            if (data.api.components.securitySchemes[ess]) {
                ...
                if (!existingAuth && ((secScheme.type === 'oauth2') || (secScheme.type === 'openIdConnect') ||
                    ...
                    if (secScheme.scheme === 'basic') {
                        authHeader.exampleValues.object = method + ' B64(API_KEY:API_SECRET)';    
                    }
                    else {
